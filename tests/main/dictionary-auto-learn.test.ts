...
import { DictionaryLearningService } from '../../src/main/dictionary/auto-learn';
...
import { DICTIONARY_LIMITS, type DictionaryCandidate } from '../../src/shared/dictionary';
...
describe('DictionaryLearningService', () => {
  it('suggests a highly confident proper term on its first observation', async () => {
    await expect(service.observe([candidate, candidate])).resolves.toEqual(candidate);
    expect(await readFile(configPath, 'utf8')).not.toContain(candidate.term);
    await expect(configuration.getDictionaryLearningState()).resolves.toMatchObject({
  ...
  it('ranks candidates by adaptive reminder priority', async () => {
    ...
    await expect(service.observe([technicalCandidate, candidate])).resolves.toEqual(candidate);
  });

  it('waits for a normal-confidence term to recur', async () => {
    await expect(service.observe([mediumConfidenceCandidate])).resolves.toBeUndefined();
    now += 1_000;
  ...
  it('silently observes a low-confidence technical term until it is stable', async () => {
    for (let observation = 1; observation < 6; observation += 1) {
      await expect(service.observe([lowConfidenceTechnicalCandidate])).resolves.toBeUndefined();
      now += 1_000;
    }

    await expect(service.observe([lowConfidenceTechnicalCandidate])).resolves.toEqual(
      lowConfidenceTechnicalCandidate,
    );
  });
  ...
  it('retains a refreshed rejection when the rejection limit is full', async () => {
    await configuration.updateDictionaryLearningState(() => ({
      candidates: [],
      rejections: Array.from({ length: DICTIONARY_LIMITS.candidates }, (_, index) => ({
    ...
    expect(state.rejections).toEqual(
      expect.arrayContaining([expect.objectContaining({ until: now + 30 * 24 * 60 * 60 * 1_000 })]),
    );
  });
  ...
  it('clears encrypted pending state when automatic learning is disabled', async () => {
    await service.observe([candidate]);
    expect((await configuration.load()).dictionaryLearning.encryptedState).toBeDefined();
