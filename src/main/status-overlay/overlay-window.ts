...
import type { ProcessResult, SupportedLanguage } from '../../core/providers/types.js';
import {
  CAPSULE_CHANNELS,
  type CapsuleErrorReason,
  type CapsuleStatus,
  type DictionarySuggestionError,
} from '../../shared/overlay-ipc.js';
  ...
  private async createAndLoadWindow(): Promise<BrowserWindow> {
    ...
    try {
      if (process.env.VITE_DEV_SERVER_URL) {
        await window.loadURL(`${process.env.VITE_DEV_SERVER_URL}/status-overlay.html`);
      } else {
        await window.loadURL('app://renderer/status-overlay.html');
      }
    } catch (error) {
